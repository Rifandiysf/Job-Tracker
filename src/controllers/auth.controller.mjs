...
async function register(req, res, next) {
    try {
        ...
        return success(res, 201, "Registrasi berhasil", result);
    } catch (err) {
        next(err);
    }
}
...
async function forgotPassword(req, res, next) {
    try {
        ...
        return success(res, 200, "Jika email terdaftar, link reset password telah dikirim");
    } catch (err) {
        next(err);
    }
}
...
async function resetPassword(req, res, next) {
    try {
        ...
        return success(res, 200, "Password berhasil direset, silakan login kembali");
    } catch (err) {
        next(err);
    }
}
